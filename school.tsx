...
const SchoolCard = React.memo(({ school }) => (
  <div
...
const ShowSchools = ({ onAddNew }) => {
  ...
  const [tempFilters, setTempFilters] = useState({ name: '', city: '', state: '' });
  const [showSuggestions, setShowSuggestions] = useState({ name: false, city: false, state: false });
  ...
  const filteredSchools = useMemo(() => {
    ...
    if (filters.name.trim()) {
      ...
      filtered = filtered.filter(school => school.name.toLowerCase().includes(n));
    }
    if (filters.city.trim()) {
      const c = filters.city.toLowerCase();
      filtered = filtered.filter(school => school.city.toLowerCase().includes(c));
    }
    if (filters.state.trim()) {
      const st = filters.state.toLowerCase();
      filtered = filtered.filter(school => school.state.toLowerCase().includes(st));
    }
    return filtered;
  }, [schools, filters]);
  ...
  const selectSuggestion = useCallback((field, value) => {
